# IP-0018 DP-0003 Implementation Plan - OPEN - High
...
- Stage Gate: A-E checklist
  ...
  - [ ] Stage C - Targeted unit/component checks
  - [ ] Stage D - DP-0003 scenario/regression runs
  - [ ] Stage E - Per-CS acceptance pass/fail
...
5. SG energy validation package (`CS-0009`): add runtime SG energy balance checks and cumulative mismatch diagnostics aligned to Stage E criteria.
6. Integrated regression and acceptance run package (`CS-0009`, `CS-0017`, `CS-0019`, `CS-0020`): execute Stage A-E gates and capture evidence artifacts per CS.
...
- Active JSON register lacks dedicated per-CS evidence artifact links for these four CS items; available references are legacy section entries and source issue labels. Evidence artifact expectations should be confirmed before execution.
- Status label mismatch exists across records (`Assigned` in legacy markdown vs `READY_FOR_FIX` in authoritative JSON). Execution should treat JSON status as authoritative.
- Stage E thresholds are defined in this plan amendment; execution kickoff must lock interval/window boundaries so threshold evaluation is deterministic.
