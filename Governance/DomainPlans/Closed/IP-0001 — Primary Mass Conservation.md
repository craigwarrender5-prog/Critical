...
---
Identifier: IP-0001
Domain: Primary Mass Conservation / Coupled Thermodynamics Solver
Status: Closed
Priority: High
Tier: 1
Linked Issues: CS-0001, CS-0002, CS-0003, CS-0004, CS-0005, CS-0006, CS-0007, CS-0008
Last Reviewed: 2026-02-14
Authorization Status: AUTHORIZED (retroactive â€” completed as v0.1.0.0)
Mode: SPEC/DRAFT (plan closed)
---

# IP-0001 â€” Primary Mass Conservation
...
| Status | CLOSED â€” Released as v0.1.0.0 |
| Constitution Reference | PROJECT_CONSTITUTION.md, Articles II, III, V, VII, IX |
...
This release establishes the foundational invariant rules that the simulator must obey going forward. The 5.x lineage is deprecated. We are not patching legacy behavior â€” we are defining the baseline authority architecture.
...
v0.1.0.0 corrects this by activating the architecture, wiring the diagnostics, and eliminating silent gates â€” making the simulator **architecturally self-verifying**.
...
The identity `RCSWaterMass + PZRWaterMass + PZRSteamMass = TotalPrimaryMass_lb` shall hold by construction at the end of every solver call in Regime 2/3. This is not checked â€” it is guaranteed by the math (Rule R5).
...
|----------|-------|----------|-------------|
| CS-0001 | CoupledThermo canonical mass mode never activated | Critical | Phase A â€” keystone |
| CS-0002 | TotalPrimaryMass_lb freezes after first step in R2/R3 | High | Phase A â€” auto-resolved |
| CS-0003 | Boundary flow accumulators never incremented | Medium | Phase B â€” accounting |
| CS-0004 | Pre-solver V*rho PZR mass computation redundant | Low | Phase D â€” cleanup |
| CS-0005 | CVCS double-count guard works by accident | Medium | Phase A/B â€” refactor |
| CS-0006 | UpdatePrimaryMassLedgerDiagnostics() dead code | High | Phase C â€” resurrection |
| CS-0007 | No UI display for primary ledger drift | Medium | Phase C â€” wiring |
| CS-0008 | No runtime solver mass conservation check | Medium | Phase B â€” guard rail |
...
- CS-0009 (SG energy balance) â€” deferred to SG domain plan
- CS-0010 (SG pressure alarm) â€” deferred to SG domain plan
- CS-0011 (acceptance tests formula-only) â€” deferred to test infrastructure plan
- CS-0012 (regime transition logging) â€” deferred to observability plan
...
3. Remove the default `0f` parameter pattern â€” make the canonical argument mandatory via enum
...
**Containment is mandatory.** Any discovered issue outside this scope shall be logged in the Issue Registry and deferred per Constitution Article V, Section 7.
...
| Constitution | Governance | All work governed by Article structure |
...
### Phase A â€” Authority Activation
...
1. **`HeatupSimEngine.cs` â€” Regime 2 call site (line ~1217)**
...
2. **`HeatupSimEngine.cs` â€” Regime 3 call site (line ~1381)**
   - Pass `physicsState.TotalPrimaryMass_lb` as the 10th argument to `BulkHeatupStep()`
   - Add authority comment: same as above

3. **`HeatupSimEngine.cs` â€” CVCS pre-application in Regime 2 (line ~1182)**
...
4. **`HeatupSimEngine.cs` â€” CVCS pre-application in Regime 3 (line ~1344)**
   - Same change as #3
   - The double-count guard (`regime3CVCSPreApplied`) remains as safety net but is now architecturally correct â€” CVCS modifies the ledger, not a component

5. **`HeatupSimEngine.cs` â€” First-step rebase (line ~1455)**
...
6. **`HeatupSimEngine.cs` â€” Regime 1 solid sync (line ~1085)**
   - No change â€” Regime 1 correctly maintains `TotalPrimaryMass_lb = TotalPrimaryMassSolid`

7. **`CoupledThermo.cs` â€” Canonical path (lines 237-272)**
   - No code changes needed â€” the canonical path is already correctly implemented
...
8. **Verification that LEGACY path is no longer reached in normal operation:**
   - Add a one-time log at `CoupledThermo.cs:123`: if `!useCanonicalMass`, log `"WARNING: CoupledThermo running in LEGACY mode â€” canonical mass not provided"`

**Acceptance Criteria â€” Phase A:**
...
- [ ] `RCSWaterMass` is computed as remainder (`M_total - PZR_water - PZR_steam`) â€” not from V*rho
...
### Phase B â€” Boundary Accounting Completion
...
1. **`HeatupSimEngine.cs` â€” Regime 2 CVCS block (lines ~1177-1188)**
...
2. **`HeatupSimEngine.cs` â€” Regime 3 CVCS block (lines ~1338-1352)**
   - Same accumulator increments as #1

3. **`HeatupSimEngine.cs` â€” Regime 1 solid CVCS block (lines ~1073-1078)**
...
5. **`RCSHeatup.cs` â€” Post-solver mass check (after line ~162)**
...
6. **Remove redundant total-mass recomputation:**
   - `HeatupSimEngine.cs:1164-1165` (Regime 2 pre-solver PZR V*rho) â€” evaluate whether to keep or remove
...
2. **`HeatupSimEngine.Logging.cs` â€” UpdatePrimaryMassLedgerDiagnostics() corrections**
   - Verify the two-phase branch (line ~492) now works correctly:
     - `primaryMassLedger_lb = TotalPrimaryMass_lb` â€” now valid (Phase A made ledger live)
     - `primaryMassComponents_lb = RCS + PZR_w + PZR_s` â€” valid (solver output)
     - `primaryMassExpected_lb = Initial + In - Out - Relief` â€” now valid (Phase B filled accumulators)
   - If any adjustment is needed for regime-awareness, apply per `DIAGNOSTIC_CALLSITE_RECOMMENDATION.md` Section 2

3. **`HeatupSimEngine.Init.cs` â€” Correct misleading comment (line ~184)**
...
4. **`HeatupValidationVisual.TabValidation.cs` â€” Add third check row**
...
5. **`HeatupSimEngine.Init.cs` â€” Change default `primaryMassStatus`**
...
1. **`RCSHeatup.cs` â€” Remove default parameter**
   ...
   - To:
     ```csharp
     float totalPrimaryMass_lb)  // MANDATORY canonical mass (INV-1)
     ```
...
2. **`CoupledThermo.cs` â€” Add solver mode announcement**
   - At line ~123, add one-time log on first call:
     ```csharp
     if (useCanonicalMass)
         Debug.Log("CoupledThermo: CANONICAL mode active â€” ledger authoritative");
     else
         Debug.LogWarning("CoupledThermo: LEGACY mode â€” V*rho mass derivation (non-canonical)");
     ```
   - Per RISK-001 mitigation: solver mode is always visible in logs

3. **Authority ownership comments â€” HeatupSimEngine.cs**
   ...
   - Format:
     ```
     // FIELD AUTHORITY â€” REGIME 3 (Full RCPs)
     // TotalPrimaryMass_lb:  ENGINE (boundary flows via CVCS pre-apply)
     // RCSWaterMass:         SOLVER (remainder = Total - PZR_w - PZR_s)
     // PZRWaterMass:         SOLVER (V*rho at equilibrium T/P)
     // PZRSteamMass:         SOLVER (V*rho at equilibrium P)
     // ORDER: CVCS -> Ledger -> Solver -> Diagnostics
     ```

4. **`CoupledThermo.cs` â€” Comment LEGACY path as explicitly deprecated**
   - At line ~274 (LEGACY else branch):
     ```
     // LEGACY MODE â€” DEPRECATED
...
This section maps mitigations to the systemic risks identified in `RISK_REGISTER.md`. Each risk reference points to the full description in the risk register â€” not duplicated here.

| Risk | Title | Mitigated By | Residual After Release |
|------|-------|-------------|------------------------|
| RISK-001 | Default Parameter Gate Pattern | Phase D (remove default, compile-time enforcement) | LOW â€” compiler prevents omission |
| RISK-002 | Dead Diagnostics Create False Confidence | Phase C (resurrect diagnostic, default "NOT_CHECKED") | LOW â€” diagnostic runs every timestep |
| RISK-003 | Formula-Only Test Suite | NOT addressed in this plan (test infra scope) | HIGH â€” unchanged, deferred |
| RISK-004 | CVCS Order-of-Operations Fragility | Phase A (CVCS -> ledger, not component) + Phase B (solver check) | LOW â€” CVCS effect is explicit, not implicit |
| RISK-005 | V*rho Last-Writer-Wins | Phase A (solver reads ledger, not component sum) + Phase D (comments) | LOW â€” solver distributes, doesn't overwrite |
| RISK-006 | Stale Ledger Cascading | Phase A (ledger is live) + Phase C (diagnostic validates) | LOW â€” ledger maintained by engine + solver |
| RISK-007 | Regime Transition Discontinuity | Phase A (cross-check at rebase) | MEDIUM â€” monitored but not structurally prevented |
| RISK-008 | Single-Step Rebase | Phase A (cross-check against Regime 1 value) | LOW â€” rebase becomes verification, not initialization |
| RISK-009 | Boundary Accumulator Incompleteness | Phase B (increment all current boundary paths) | MEDIUM â€” future paths must add increments |
| RISK-010 | SG Model Trust | NOT addressed in this plan (SG domain scope) | MEDIUM â€” unchanged, deferred |

**Post-release risk posture:** 6 of 10 risks reduced to LOW. 2 remain MEDIUM (regime transition, boundary completeness â€” mitigated but require ongoing vigilance). 2 remain HIGH/MEDIUM (formula-only tests, SG trust â€” deferred to future plans).
...
|-----------|-----------|-------------|
| Conservation identity (INV-4) | `|componentSum - ledger| < 0.01 lb` every timestep | BLOCK â€” cannot release |
| 8-hour drift (V-06) | < 1 lb | BLOCK â€” cannot release |
| Regime transition mass jump | < 50 lb | BLOCK â€” investigate |
| Diagnostic false positive | 0 occurrences during V-04, V-06 | BLOCK â€” fix diagnostic |
| Solver LEGACY warning | 0 occurrences in normal runs | BLOCK â€” call site not updated |
| Compile without 10th argument | Must fail compilation | BLOCK â€” default not removed |

---

## 9. EXIT CRITERIA

Per Constitution Article V, Section 8:

- [x] All assigned issues (CS-0001 through CS-0008) are Closed or Deferred with justification
- [x] All Phase A-D acceptance criteria pass
- [x] Verification protocol scenarios V-01 through V-06 recorded with metrics
- [x] All pass/fail criteria met (no BLOCK conditions)
- [x] Issue Registry (`Critical/Updates/ISSUE_REGISTRY.md`) created and current
- [x] Changelog written (`Critical/Updates/Changelogs/CHANGELOG_v0.1.0.0.md`)
- [x] Version increment performed per Constitution Article II
...
| Relief valve accumulator may remain 0f | If no relief physics exists, `CumulativeReliefMass_lb` stays at 0f | Acceptable â€” accumulator is structurally ready, incremented when relief physics is implemented |
| RISK-003 (formula-only tests) remains HIGH | No runtime acceptance test validation | Acknowledged â€” adds runtime diagnostics as a partial mitigation |
