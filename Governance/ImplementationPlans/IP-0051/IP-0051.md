...
This IP addressed visualization gaps identified during cold startup monitoring review. Changes are display-only with no physics impact. The implemented scope includes:

1. T_pzr arc gauge in PZR column (50-600°F range, color-coded for saturation)
2. T_pzr readout removed from RCS column (now displayed via PZR arc gauge)
3. T_pzr sparkline trend (replaced NET HEAT in slot 7)
