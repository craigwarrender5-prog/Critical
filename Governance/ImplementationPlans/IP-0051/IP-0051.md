...
2. **T_pzr readout misplaced** — Currently in RCS column instead of Pressurizer column
3. **No T_pzr sparkline trend** — Cannot observe heating trajectory toward saturation over time
...
|-------|-------|----------|--------|
| CS-0111 | PZR temperature (T_pzr) lacks primary visualization — no arc gauge or trend display | MEDIUM | READY |
...
**LOW** — Changes are isolated to display rendering. No physics modules affected.

### 3.5 Critical Path
Single CS. No parallel execution needed.
...
| Gauge range 50-600°F appropriate for startup | Code review |
| Color indication based on saturation approach | Visual: cyan (subcooled) → green (at saturation) |
...
| Existing PZR ΔT_SAT readout still visible | Visual inspection |
...
- Range 50-600°F covers cold startup (100°F) through saturation (450°F) with margin
...
2. **Update Initialize()** method — replace NET HEAT configuration with T_PZR:
```csharp
// T_pzr: 50-600°F (replaces NET HEAT)
...
- Range 50-600°F matches the arc gauge
...
4. Update CS-0111 status to CLOSED
5. Update IP-0051 status to CLOSED
6. Create changelog entry (CHANGELOG_v1.0.2.0.md)
...
None. CS-0111 is fully addressed by this IP.
...
2. T_pzr readout relocation (RCS → PZR column removal)
